# Handle Large JSON Messages

Some broker services like Kafka have a limit on the size of the payload.
If the message is larger than the value accepted by the broker, then a producer will fail to send the message.
One of the ways  to handle this issue is to chunk large messages into smaller chunks which don't exceed the max accepted 
size of the message by the broker.
Consumers should be aware that messages are chunked and they have to wait to get all chunks before they
can construct the original message back.
But before splitting the original message into the chunks and sending its chunks as a part of the new JSON messages, 
the whole message should be encode to a format that will not break json.
The are several ways how it can be done, one of them is to encode the original message into base64 string. Another way is
to convert original message to escaped JSON string. In the following example we will use base64 encoding, but code have
both solution, you can decide to use either Base64ChunkProducer or EscapedJsonChunkProducer classes.


For example,if we have a very large message of size 2.5Mb, the solution is to split this message to 3 smaller chunks,
where each chunk is not larger than 1Mb


Original message:
```json
{
  "name": "some name",
  "id": "some id",
  "picture": "some big data"
}
```

Chunk #1:
```json
{"chunk_data": "YXRhdC4gQWxp==",
 "chunk_position": 0,
 "chunk_total": 3,
 "message_id": "8672ed87-ea7d-4d1e-b3f7-b0affcd89003"
}
```

Chunk #2:
```json
{"chunk_data": "IGluIGF1dGUgY3VwaWRhdGF0IHZ==",
 "chunk_position": 1,
 "chunk_total": 3,
 "message_id": "8672ed87-ea7d-4d1e-b3f7-b0affcd89003"
}
```
Chunk #3:
```json
{"chunk_data": "bHRlciBMZXZ5ISBZb3UgaGF2ZSAzIHV==",
 "chunk_position": 2,
 "chunk_total": 3,
 "message_id": "8672ed87-ea7d-4d1e-b3f7-b0affcd89003"
}
```

The main idea behind this python code is to provide an ability to chunk large message into small chunks

## Usage

```python
import json
from uuid import uuid4
from chunk.chunker import Base64ChunkProducer

# chunk size 1Kb
CHUNK_SIZE = 2 ** 10

message_data = {
    "name": "James Bond",
    "id": "007",
    "picture": "data:image/png;base64,"
}
message = json.dumps(message_data)
chunk_producer = Base64ChunkProducer(
    message, message_id=str(uuid4()), chunk_size=CHUNK_SIZE
)
chunks = []
for chunk in chunk_producer.to_chunks():
    # send your chunk or add it to some list
    chunks.append(chunk)
# verify the original message is the same as message that created from chunks
message_from_chunks = chunk_producer.from_chunks(chunks)
assert message_from_chunks == message


```
